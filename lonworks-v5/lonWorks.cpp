...
#include "lonWorks.hpp"

using namespace std;

void lonWorks::control()
{
	...
	socketStatus = getaddrinfo(NULL, "1104", &host_info, &host_info_list);
	if (socketStatus != 0)
	{
...
int main()
{
	try
	{
		...
		thread th1(&lonWorks::control,lonWorks());
		th1.join();
		//lonWorks l1{};
		//l1.control();		
    }
    ...
    catch(string errMsg) //Takes a reference to an 'exception' object
    {
		cout << errMsg;
    }
	return 0;
