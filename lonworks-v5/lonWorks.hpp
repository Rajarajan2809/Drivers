/*******************************************************************************
 *	Purpose		:	Lonworks AC Integration.							   	   *
...
bool lonWorks::ctrlAckRcvd=0;
bool lonWorks::fbAckRcvd=0;
...
//modeSet
bool lonWorks::modeSet(string locInsId,string mode)
{
...
bool lonWorks::tempSet(string locInsId,string temp)//local instance id,temp
{
	//format : #[ac no]@[command no]@[request data]$
	string acId{},cmd{};
	testMysql db("Milan_SP");
...
bool lonWorks::fanSet(string locInsId,string fan)//local instance id,fan
{
	//format : #[ac no]@[command no]@[request data]$
	string acId{},cmd{};
	testMysql db("Milan_SP");
...
bool lonWorks::fb(string locInsId)//fb
{
	//format : #[ac no]@[command no]@[request data]$
	string acId{},cmd{};
	testMysql db("Milan_SP");
 ...
 * ****************************************************************************/
bool lonWorks::socketSend(string ip,string port, string lwCmd, string &lwResp)
{
 ...
 ****************************************************************************************/
void lonWorks::curlRequest(string webQuery,string type)
{
	#ifdef DEBUG
	...
	CURL *curl;
    CURLcode res;
    curl_easy_escape(curl,webQuery,webQuery.length());
	string curlResponse{};
...
string lonWorks::getMyIp(string interface)
{
	//declaration
	int fd;
	string ip{};
...
//check for integer
bool lonWorks::isInt(const string& s, int base)
{
