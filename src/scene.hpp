...
/*******************************************************************************
 * Class which perform Scene execution.		                                   *
 *******************************************************************************/
class scene
{
	public:
			...
			scene();
			~scene();
			int sceneExecution(string userId,string sceneId);
			string getMyIp(string interface);
			int socketSend(string jsonData,string port);
			...
			friend size_t CurlWrite_CallbackFunc_StdString(void *contents, size_t size, size_t nmemb, string *s);
			int webQuery(string url,string type);
...
scene::scene():db("Milan_SP")
{
	/*-----Progress flag is set and Stop flag is reset-----*/
	#ifdef DEBUG
		cout << "Scene Execution Started at:" << getTime() << br() << br();
	#endif
}
...
scene::~scene()
{
	...
	#ifdef DEBUG
		cout << "Scene Execution finished at:" << getTime() << br() << br();
	#endif
}
...
//json creation
string scene::jsonCreate(string category,string userId,string localInstanceId,string integrationId,string param1,string param2,string param3,string param4,string param5,string param6,string param7,string param8,string param9,string param10)
{
...
//to json server
int scene::socketSend(string jsonData,string port)
{
	...
	socketStatus = getaddrinfo("localhost", port.c_str(), &host_info, &host_info_list);
	if (socketStatus != 0)
	{
		/*-----Could not get address info.-----*/
		#ifdef DEBUG
			cout << "JSON Server(mass-exec):Could not get address info." << br();
		#endif
	...
	else
	{
		...
		if (socketFd == -1)  
		{
			/*-----socket error-----*/
			#ifdef DEBUG
				cout << "JSON Server:socket error." << br();
			#endif
			freeaddrinfo(host_info_list);
		...
		else
		{
			...
			if (connectStatus == -1) 
			{
				/*-----not connected-----*/
				#ifdef DEBUG
					cout << "JSON Server:not connected." << br();
				#endif
				close(socketFd);
				freeaddrinfo(host_info_list);
...
void timerSecs(int timeInSecs,string param1,string param2,string param3,string param4,string param5,string param6,string param7,string param8,string param9,string param10)
{
	...
	scene s1;
	if((param1 == "IR") || (param1 == "Channels"))
	{
		sleep(timeInSecs);
		string url = "http://localhost:6161/Milan/Drivers/MILAN_EXE/IR/ir.cgi?";
		if(param1 == "IR")
		{
			url = url+"l_instid="+param3+"&irid="+param5+"&cat="+param1+"&userid="+param2+"&cmd1="+param6+"&cmd2=NA&cmd3=NA&cmd4=NA";
		}
		else if(param1 == "channels")
		{
			url = url+"l_instid="+param3+"&irid="+param5+"&cat="+param1+"&userid="+param2+"&cmd1=NA&cmd2=NA&cmd3="+param6+"&cmd4=NA";
		}
		s1.webQuery(url,"GET");
	}
	else
	{
		sleep(timeInSecs+2);
		#ifdef DEBUG
			cout<<"Thread json:"<<s1.jsonCreate(param1,param2,param3,param4,param5,param6,param7,param8,param9,param10,"","","","") << br() << br();
		#endif
		s1.socketSend(s1.jsonCreate(param1,param2,param3,param4,param5,param6,param7,param8,param9,param10,"","","",""),"64527");
	}
}
...
string scene::getMyIp(string interface)
{
	//declaration
	int fd;
	string ip{};
...
int scene::webQuery(string url,string type)
{
	CURL *curl;
	CURLcode res;
	...
	string curlResponse{};
	#ifdef DEBUG
		cout << "web query:" << url << br() << "type:" << type << br();
	#endif
	if(curl)
	{
	...
	#ifdef DEBUG
		cout << "curl response:" << curlResponse << br() << br();
	#endif
	return 0;
}
#endif
